...
class EstudianteRepository:
    ...
    async def mostrarEstudiantes(self):
        ...
        cursor.execute("""
            SELECT id, usuario_id, padre_id, grado, colegio, distrito
            FROM estudiantes
        """)
    ...
    async def agregarEstudiante(self, estudiante: Estudiante):
        ...
        cursor.execute("""
            INSERT INTO estudiantes (usuario_id, padre_id, grado, colegio, distrito)
            VALUES (?, ?, ?, ?, ?)
        """, (
    ...
    async def actualizarEstudiante(self, id: int, estudiante: EstudianteActualizar):
        ...
        cursor.execute("""
            UPDATE estudiantes
            SET usuario_id = ?, padre_id = ?, grado = ?, colegio = ?, distrito = ?
            WHERE id = ?
        """, (
            estudiante.usuario_id,
            estudiante.padre_id,
